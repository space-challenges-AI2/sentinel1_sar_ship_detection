...
Multi-GPU distributed training commands:
...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    ...
    # Load pretrained weights if available, otherwise not
    if pretrained:
        with torch_distributed_zero_first(LOCAL_RANK):
    ...
    # Check if mixed precision training is possible
    ...
    # Distributed training mode: DP mode
    if cuda and RANK == -1 and torch.cuda.device_count() > 1:
        LOGGER.warning('WARNING ⚠️ DP not recommended, use torch.distributed.run for best DDP Multi-GPU results.\n'
                       'See Multi-GPU Tutorial at https://github.com/ultralytics/yolov5/issues/475 to get started.')
        model = torch.nn.DataParallel(model)

    # SyncBatchNorm: synchronize BN layers
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        # Add training output Logger information
        LOGGER.info(('\n' + '%11s' * 7) % ('Epoch', 'GPU_mem', 'box_loss', 'obj_loss', 'cls_loss', 'Instances', 'Size'))
        if RANK in {-1, 0}:
            pbar = tqdm(pbar, total=nb, bar_format=TQDM_BAR_FORMAT)  # progress bar
        # Clear gradients before optimizer training
        optimizer.zero_grad()
        # Start training iteration
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            callbacks.run('on_train_batch_start')
            ni = i + nb * epoch  # number integrated batches (since train start)
            # Normalize images and load to GPU
            ...
            # Multi-scale: perform multi-scale training
            if opt.multi_scale:
            ...
            # Forward: perform forward inference
            ...
            # Backward: perform loss backpropagation
        ...
        if RANK in {-1, 0}:
            ...
            # Calculate mAP, i.e., run val function
        ...
        # end epoch ----------------------------------------------------------------------------------------------------
    # end training -----------------------------------------------------------------------------------------------------
    # End training and perform final metric validation
    if RANK in {-1, 0}:
...
def parse_opt(known=False):
    ...
    parser.add_argument('--data', type=str, default=ROOT / 'data/HRSID_land.yaml', help='dataset.yaml path')
    parser.add_argument('--hyp', type=str, default=ROOT / 'data/hyp/hyp.scratch-low.yaml', help='hyperparameters path')
    ...
    parser.add_argument('--name', default='experiment', help='save to project/name')
