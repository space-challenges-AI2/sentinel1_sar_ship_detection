...
class RFAConv(nn.Module):  # RFAConv based on Group Conv implementation
    ...
    def forward(self, x):
        ...
        feature = self.generate_feature(x).view(b, c, self.kernel_size ** 2, h,
                                                w)  # b c*kernel**2,h,w ->  b c k**2 h w   Get receptive field spatial features
...
class RFAConv_U(nn.Module):  # RFAConv based on Unfold implementation
    ...
    def forward(self, x):
        b, c, h, w = x.shape
        unfold_feature = self.unfold(x)  # Get receptive field spatial features  b c*kernel**2,h*w
        x = unfold_feature
